...
document.querySelector('.clock-container').addEventListener('click', function handler() {
    Run();
    document.querySelector('.clock-container').removeEventListener('click', handler);
});
...
fetch('./configs.json')
    ...
    .then(data => {
        ...
        setInterval(() => {
            ...
            if (data.celebration_days == daysCount && trigger) {
                Run();
                trigger = false;
            }
        }, 1000)
